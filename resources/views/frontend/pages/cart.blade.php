...
$(document).ready(function() {
    // Only handle quantity change to update totals
    $('.input-number').on('change', function() {
        let row = $(this).closest('tr');

        // Get unit price
        let unitPrice = parseFloat(row.find('.price span').text().replace('$', ''));

        // Get quantity
        let qty = parseInt($(this).val());

        // Calculate row total
        let rowTotal = (unitPrice * qty).toFixed(2);

        // Update row total display
        row.find('.cart_single_price span').text('$' + rowTotal);

        // Recalculate cart subtotal
        let subtotal = 0;
        $('.cart_single_price span').each(function() {
        ...
        $('#order_total_price span').text('$' + (subtotal - coupon).toFixed(2));
    });
});
</script>
